...
const checkUserByName = (name) => {
  return pool.query(`SELECT * FROM users WHERE users.fullname ILIKE '%${name}%' `);
};
...
const createUser = (data) => {
  ...
  return new Promise((resolve, reject) =>
    pool.query(`INSERT INTO users(id, email, fullname, password, otp) VALUES('${id}', '${email}', '${fullname}', '${password}', ${otp})`, (err, res) => {
      if (!err) {
        resolve(res);
      } else {
        reject(err.message);
...
module.exports = { selectUser, updateUser, checkUserById, userQuery, createUser, verifyUser, deleteUser, checkUserByName };
